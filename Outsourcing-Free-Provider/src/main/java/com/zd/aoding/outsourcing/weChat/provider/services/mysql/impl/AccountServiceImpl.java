...
public class AccountServiceImpl implements AccountService {
	@Autowired
	private BaseDAO<AccountDO, Integer> baseDAO;
	private String name = "账号（account）";
	private static final String nameSpace = AccountDO.class.getName();
	...
	@Override
	public int update(AccountDO po) {
	...
	@Override
	public AccountDO get(Integer primaryKey) {
	...
	public List<AccountDO> getList(Map<String, Object> param) {
		...
		try {
			List<AccountDO> list = baseDAO.getList(param,nameSpace);
			return list;
		} catch (Exception e) {
	...
	@Override
	public List<AccountDO> getPagination(PageEntity page) {
		
		try {
			List<AccountDO> list = baseDAO.getPagination(page,nameSpace);
			return list;
		} catch (Exception e) {
	...
	@Override
	public int insert(AccountDO po) {
	...
	@Override
	public List<AccountDO> getBySQLAdapter(SQLAdapter adapter) {
		
		try {
			List<AccountDO> list = baseDAO.getCustom(adapter,nameSpace);
			return list;
		} catch (Exception e) {
			LOGGER.error(name + "根据所有满足条件获取分页数据失败：", e);
			e.printStackTrace();
