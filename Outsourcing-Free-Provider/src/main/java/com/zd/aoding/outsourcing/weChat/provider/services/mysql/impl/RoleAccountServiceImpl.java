...
public class RoleAccountServiceImpl implements RoleAccountService {
	@Autowired
	private BaseDAO<RoleAccountDO, Integer> baseDAO;
	private String name = "用户角色（RoleAccountDO）";
	private static final String nameSpace = RoleAccountDO.class.getName();
	private static final Logger LOGGER = Logger.getLogger(RoleAccountServiceImpl.class);
	...
	@Override
	public int update(RoleAccountDO po) {
	...
	@Override
	public RoleAccountDO get(Integer primaryKey) {
	...
	@Override
	public List<RoleAccountDO> getList(Map<String, Object> param) {
	
		try {
			List<RoleAccountDO> list = baseDAO.getList(param,nameSpace);
			return list;
		} catch (Exception e) {
	...
	@Override
	public List<RoleAccountDO> getPagination(PageEntity pageEntity) {
	
		try {
			List<RoleAccountDO> list = baseDAO.getPagination(pageEntity,nameSpace);
			return list;
		} catch (Exception e) {
	...
	@Override
	public int insert(RoleAccountDO Entity) {
	...
	public List<RoleAccountDO> getBySQLAdapter(SQLAdapter adapter) {
	
		try {
			List<RoleAccountDO> list = baseDAO.getCustom(adapter,nameSpace);
			return list;
		} catch (Exception e) {
			LOGGER.error(name + "根据所有满足条件获取分页数据失败：", e);
			e.printStackTrace();
