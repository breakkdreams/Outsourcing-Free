	...
	table = $("#dealerTable").dataTable(
			{
				...
				"columnDefs" : [
					...
					"data" : "id",
					"render" : function(data, type, row) {
						var html = "<a onclick='updateBusiness("
								+ row["accountId"] + ")'>修改</a>&nbsp;|&nbsp;<a onclick='resetPassword("
                            + row["accountId"] + ")'>重置密码</a>&nbsp;|&nbsp;<a onclick='deleted("
                            + row["accountId"] + ")'>删除</a>";
						return html;
					}
				} ]
...
function addBusiness() {
	window.location.href=businessAddPage;
}
//充值密码
function resetPassword(id) {
    showMsg(restps, id, "你确定要重置密码吗？");
}
...
function updateBusiness(id) {
    window.location.href=businessEditPage+"?accountId="+id;
}
...
// 删除记录
function deleted(id) {
    showMsg(deleteAction, id, "你确定要删除该商户吗？");
}
// 删除调用方法
